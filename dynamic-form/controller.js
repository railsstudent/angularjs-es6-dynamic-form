import { get, filter, forEach, chain } from 'lodash-es';
import $ from 'jquery';
...
class DynamicFormController {
  constructor($compile, $scope, DynamicFormService, $timeout) {
    ...
    this.$onInit = () => {
      ...
      if (formEl) {
        ...
        $img.on('error', e => $timeout(() => this.loading.image = false, 500));
        chain(this.data)
  ...
  changeCountry() {
    ...
    if (country) {
      this.object.image = `https://countryflags.io/${this.object.country.code}/shiny/64.png`;
